...
values ('PT1701000000000006JQ', 'Alfândega da Fé', 20, 85, 20, 50, 41.33134454715996, -6.968427934882252,
        'Alfândega da Fé', (SELECT dno.id FROM dno WHERE dno.name = 'Sonorgas')),
       ('PT1701000000000009JL', 'Alijó', 30, 85, 20, 20, 41.29741788010863, -7.477619913492303,
        'Alijó', (SELECT dno.id FROM dno WHERE dno.name = 'Sonorgas')),
       ('PT1701000000000012JE', 'Carrazeda de Ansiães', 25, 85, 20, 52, 41.24742767159777, -7.317666075364068,
        'Cazerrada de Ansiães', (SELECT dno.id FROM dno WHERE dno.name = 'Sonorgas'));

INSERT INTO tank (agu_cui, number, min_level, max_level, critical_level, load_volume, correction_factor, capacity)
values ('PT1701000000000006JQ', 1, 20, 85, 20, 50, 1.5, 100),
       ('PT1701000000000009JL', 1, 30, 85, 20, 20, 1.0, 80),
       ('PT1701000000000012JE', 1, 25, 85, 20, 52, 0.5, 120);
