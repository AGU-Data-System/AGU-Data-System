...
import aguDataSystem.server.domain.TrainingModel
import aguDataSystem.server.domain.measure.*
...
class FetchService(
    ...
    fun fetchAndSave(provider: Provider, since: LocalDateTime) {
        val providerURL =
            Environment.getFetcherUrl() + "/provider/${provider.id}?beginDate=$since&size=$pageSize"
    ...
    fun generatePredictions(
        ...
        val trainingDataString = objectMapper.decodeFromString<JsonObject>(training)["training"]!!.jsonPrimitive.content
        val trainingData = objectMapper.decodeFromString<TrainingModel>(trainingDataString)

        val coefficients = trainingData.coefficients
        val intercept = trainingData.intercept
