...
class TransportCompanyController(private val service: TransportCompanyService) {
    ...
    fun addTransportCompany(
        ...
        return when (val res = service.addTransportCompany(transportCompanyCreationInputModel.toTransportCompanyCreationDTO())) {
            is Failure -> res.value.resolveProblem()
    ...
    private fun GetTransportCompaniesOfAGUError.resolveProblem(): ResponseEntity<*> {
        return when (this) {
            GetTransportCompaniesOfAGUError.AGUNotFound -> Problem.response(HttpStatus.NOT_FOUND.value(), Problem.AGUNotFound)
        }
    }
